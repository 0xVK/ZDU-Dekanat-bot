...
bot = telebot.TeleBot('token_code')
...
def add_user_to_db(chat, group):

    if not check_user_if_exist(chat.id):  # create new user

        with open('db.txt', 'a') as user_db_file:
            user_db_file.write('{}:{}:({} {}):{}\n'.format(chat.id, chat.username,
                                                           chat.first_name, chat.last_name, group))
...
def set_group(message):
    ...
    add_user_to_db(message.chat, message.text)
